...
const formSchema = z.object({
  ...
  rating: z
    ...
    .optional()
    .refine(
      (v) => !v || (!Number.isNaN(Number(v)) && Number(v) >= 0 && Number(v) <= 10),
      'Puntuación debe ser un número entre 0 y 10',
    ),
  year: z
    .string()
    .optional()
...
export function MovieForm({
  ...
  const submitHandler = (values: FormValues) => {
    const payload: CreateMovieDto = {
      ...
      rating:
        values.rating && values.rating.toString().trim().length > 0
          ? Number(values.rating)
          : undefined,
      year:
        values.year && values.year.toString().trim().length > 0 ? Number(values.year) : undefined,
    ...
    <form onSubmit={handleSubmit(submitHandler)} className="space-y-4" noValidate>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          />
          {errors.poster_path && (
            <p className="mt-1 text-xs text-red-400">{errors.poster_path.message}</p>
          )}
        </div>
