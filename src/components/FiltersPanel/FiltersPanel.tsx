...
export const FiltersPanel = ({ availableGenres, initial, onChange, onApply, onReset }: Props): React.JSX.Element => {
    ...
    useEffect(() => {
        const filters: Filters = {
            ...
            rating:
                (minRating !== '' || maxRating !== '')
                    ? { min: minRating === '' ? undefined : Number(minRating), max: maxRating === '' ? undefined : Number(maxRating) }
                    : undefined,
            year: year || undefined,
        };
        onChange?.(filters);
    ...
    return (
        <aside className="p-4 bg-gray-900 border border-gray-800 rounded-md space-y-4">
            <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                    {availableGenres.map((g) => {
                        ...
                        return (
                            <label key={g} className={`inline-flex items-center px-2 py-1 rounded-md cursor-pointer ${checked ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-200'}`}>
                                <input
                                    type="checkbox"
                                    checked={checked}
            ...
            <div className="flex items-center gap-2 justify-end">
                <button type="button" onClick={handleReset} className="px-3 py-1 rounded-md bg-gray-700 text-gray-100 hover:bg-gray-600">
                    Reiniciar
                </button>
                <button type="button" onClick={handleApply} className="px-3 py-1 rounded-md bg-indigo-600 text-white hover:bg-indigo-500">
                    Aplicar
                </button>
            </div>
        </aside>
